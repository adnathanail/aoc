...
def combine_maps(map1, map2):
    ...
    for start1, end1, value1 in map1:
        ...
        for start2, end2, value2 in map2:
            ...
            # s2|         e2|
            if (
                    (start2 <= s1p and s1p < end2 < e1p) or
                    (s1p < end2 and end2 < e1p) or
                    (s1p <= start2 and start2 < e1p) or
                    (start2 <= s1p and e1p <= end2)
            ):
                combined_map.append([max(s1p, start2) - value1, min(e1p, end2) - value1, value1 + value2])
