    ...
    for i in 0..10 {
        ...
        for i in distance_lookup.keys() {
            for j in distance_lookup[i].keys() {
                // println!("{} {} {}", i, j, distance_lookup[i][j]);
        ...
        // println!("{:?} {}", closest_points, shortest_distance);
        connections.push(closest_points.unwrap());
        connected_components.union(closest_points.unwrap().0, closest_points.unwrap().1);
    }
    // let x: HashSet<&usize> = HashSet::from_iter(connected_components.to_vec());
    // println!("{:?}", connected_components);
    // println!("{:?}", connected_components.to_vec());
